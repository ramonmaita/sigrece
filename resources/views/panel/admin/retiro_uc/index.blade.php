...
@section('content')
	  ...
	  {{-- @dump($periodo = \App\Models\User::find(1)->audits()->with('user')->get())
	  @dump($user = \DB::table('audits')->where('user_id',1)->get()) --}}
	  {{-- @foreach ($collection as $item)

	  @endforeach --}}

	  {{-- @dump($user->first()->new_values)
		{{ $user->first()->new_values }} --}}
	{{-- @dump($periodo->getMetadata()) --}}

	{{-- TODO: NO BORRAR ALLI ESTA COMO HACER EL MODULO DE AUDITORIAS --}}
	@include('alertas')
	<div class="card card-primary card-outline">
		<div class="card-header">
			<div class="float-right">
				<a class="btn btn-primary" href="{{ route('panel.retiros.create') }}" role="button">
					Nueva Solicitud
				</a>
			</div>
		</div>
