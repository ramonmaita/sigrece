...
{{-- 	<br> --}}
	...
	<p style="text-align: justify;">
		{{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'Al ' : 'A la'  }} Ciudadan{{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'o' : 'a'  }}: <span class="datos" style="text-transform: capitalize !important;"> {{ ucwords(mb_strtolower(' '.$graduando->Alumno->p_apellido,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$graduando->Alumno->s_apellido,"UTF-8" )) }}, {{ ucwords(mb_strtolower(' '.$graduando->Alumno->p_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$graduando->Alumno->s_nombre,"UTF-8" )) }}  </span> titular  {{ ($graduando->Alumno->nacionalidad == 'V') ? 'de la Cédula de Identidad' : 'del Pasaporte'  }} número: <span class="datos" style="font-size: 12pt !important">{{ ($graduando->Alumno->nacionalidad == 'V') ? $graduando->Alumno->nacionalidad.'-'.$graduando->Alumno->cedula : $graduando->Alumno->cedula  }}</span>, {{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'nacido' : 'nacida'  }} en <span class="datos" style="text-transform: capitalize !important;"> {{ mb_strtolower( $graduando->Alumno->lugarn,"UTF-8" ) }}</span>, el  <span class="datos" style="font-size: 12pt !important">{{ Carbon::parse($graduando->Alumno->fechan)->format('d/m/Y')}}</span>, por haber aprobado el plan de estudios en tal opción y haber cumplido con los requisitos de ley que rigen la materia. Para constancia se levanta la presente Acta y firma el Rector, la Secretaria y el Graduando.
	</p>
