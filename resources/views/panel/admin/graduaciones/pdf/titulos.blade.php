	...
	<div class="cont" style="background-color:rgba(255, 255, 255, 0.9); background-image: url({{ asset('/img/fondo-titulo.png') }});">
		...
		<div class="center-img">
			...
			<p style="margin: 0px; font-size: 16pt;">
				Que {{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'el' : 'la'  }} Ciudadan{{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'o' : 'a'  }}{{ ($graduando->titulo == 1 ||  $graduando->titulo == 3) ? ':' : ':'  }}
			</p>
			<div style=" /*font-family: 'gregorian';*/ font-size: 26pt; text-align: center; margin-top: 0px; margin-bottom: 0px;">
				<center style="text-transform: capitalize !important;">
					{{ ucwords(mb_strtolower(' '.$graduando->Alumno->p_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$graduando->Alumno->s_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$graduando->Alumno->p_apellido,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$graduando->Alumno->s_apellido,"UTF-8" )) }}
				</center>
			</div>
			<p align="center" style="font-size: 16pt; margin: 0px; ">
				Titular {{ ($graduando->Alumno->nacionalidad == 'V') ? 'de la Cédula de Identidad' : 'del Pasaporte'  }} Número {{ ($graduando->Alumno->nacionalidad == 'V') ? $graduando->Alumno->nacionalidad.'-'.$graduando->Alumno->cedula : $graduando->Alumno->cedula  }}, natural de <span style="text-transform: capitalize !important;">{{ mb_strtolower( $graduando->Alumno->lugarn,"UTF-8" ) }}</span>, {{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'Nacido' : 'Nacida'  }} el {{ Carbon::parse($graduando->Alumno->fechan)->format('d/m/Y')}}, aspirante al Título de
			...
			@endif
				<center>
					@if ($graduando->titulo == 1)
						@php
							echo $retVal = ($graduando->Alumno->sexo == 'M') ? '<span class="inicial">T</span>écnico <span class="inicial">S</span>uperior <span class="inicial">U</span>niversitario en:' : '<span class="inicial">T</span>écnica <span class="inicial">S</span>uperior <span class="inicial">U</span>niversitaria en:';
						@endphp
			...
			<p align="center" style="font-size: 16pt; margin: 0px; margin-top:5px !important">
				Tómese razón de este título en la Secretaría de la Universidad, y Reconózcase  en toda la República {{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'al' : 'a la'  }} Ciudadan{{ $retVal = ($graduando->Alumno->sexo == 'M') ? 'o' : 'a'  }}
