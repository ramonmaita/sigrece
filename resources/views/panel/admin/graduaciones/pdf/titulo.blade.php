	...
	<div class="cont" style="background-color:rgba(255, 255, 255, 0.9); background-image: url({{ asset('/img/fondo-titulo.png') }});">
		...
		<div class="center-img">
			...
			<p style="margin: 0px; font-size: 16pt;">
				Que {{ $retVal = ($alumno->sexo == 'M') ? 'el' : 'la'  }} Ciudadan{{ $retVal = ($alumno->sexo == 'M') ? 'o' : 'a'  }}{{ ($graduando->titulo == 1 ||  $graduando->titulo == 3) ? ':' : ':'  }}
			</p>
			<div style=" /*font-family: 'gregorian';*/ font-size: 26pt; text-align: center; margin-top: 0px; margin-bottom: 0px;">
				<center style="text-transform: capitalize !important;">
					{{-- {{ ucwords(mb_strtolower($alumno->p_nombre,"UTF-8" )) }} --}}
			...
			@php
				$p = $graduando->ira;
				// && Carbon::parse($graduando->egreso)->format('d-m-Y') < '4-07-2019' || $p >= 18.00 && $p <= 19.49 && Carbon::parse($graduando->egreso)->format('d-m-Y') > '4-07-2019' && $graduando->titulo == 2
			...
			<p align="center" style="font-size: 16pt; margin: 0px; margin-top:5px !important">
				Tómese razón de este título en la Secretaría de la Universidad, y Reconózcase  en toda la República {{ $retVal = ($alumno->sexo == 'M') ? 'al' : 'a la'  }} Ciudadan{{ $retVal = ($alumno->sexo == 'M') ? 'o' : 'a'  }}

				{{-- <span style="text-transform: capitalize !important;">

				{{ ucwords( mb_strtolower(' '.$alumno->p_nombre,"UTF-8" ),'  ') }} {{ ucwords(mb_strtolower(' '.$alumno->s_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->p_apellido,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->s_apellido,"UTF-8"))}}</span>, como tal
				...
				<br>
				En fé de lo cual Firmo el presente título en unión de la Secretaria y un Profesor. En Ciudad Bolívar, a los {{ strtolower($dia) }} días del mes de {{ strtolower($mes) }} del año dos mil {{ strtolower($graduando->numero_letras(Carbon::parse($graduando->egreso)->format('y'))) }}. Año {{Carbon::parse($graduando->egreso)->format('Y') - 1810 }} de la Independencia y {{Carbon::parse($graduando->egreso)->format('Y') - 1859 }} de la Federación.
			</p>
			<br>
			<br>
