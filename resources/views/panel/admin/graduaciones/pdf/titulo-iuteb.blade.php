...
<body >
	...
	<div class="cont" style="background-color:rgba(255, 255, 255, 0.9); background-image: url({{ asset('/img/titulo_acta/fondo-titulo-iuteb.png') }});">
		...
		<div class="center-img">
			<table border="0"  align="center" cellpadding="0" cellspacing="0" style="margin-top: -20px;" width="100%">
				<tr>
					...
					<td align="right" width="25%">
						<img src="data:image/png;base64,{{DNS2D::getBarcodePNG('https:iuteb.edu.ve/sigce/public/'.$graduando->nacionalidad.'/'.$graduando->cedula.'/verificar/'.$graduando->nro_titulo, 'QRCODE')}}" alt="barcode" height="120px" width="120px"  style="align: right " />
					</td>
				</tr>
				<tr>
					<td colspan="3">
			...
			<span style="font-size: 16pt; text-align: center;">
				<center>
					...
					<p style="font-size: 33pt; text-align: center; padding-top: 0px !important; line-height: 20px; margin: 10px; margin-bottom: 10px;">
						<span class="inicial">I</span>nstituto <span class="inicial">U</span>niversitario de <span class="inicial">T</span>ecnología del <span class="inicial">E</span>stado <span class="inicial">B</span>olívar
					</p>
				</center>
			</span>
			...
			<p style="margin: 0px; font-size: 16pt;">
				Que {{ $retVal = ($alumno->sexo == 'M') ? 'el' : 'la'  }} Ciudadan{{ $retVal = ($alumno->sexo == 'M') ? 'o' : 'a'  }}{{ ($graduando->titulo == 1 ||  $graduando->titulo == 3) ? ' Bachiller:' : ':'  }}
			</p>
			<div style=" /*font-family: 'gregorian';*/ font-size: 26pt; text-align: center; margin-top: 0px; margin-bottom: 0px;">
				<center style="text-transform: capitalize !important;">
					{{-- {{$graduando->nombres}} {{$graduando->apellidos}} --}}
					{{ ucwords( mb_strtolower(' '.$alumno->p_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->s_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->p_apellido,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->s_apellido,"UTF-8" )) }}
				</center>
			</div>
			<p align="center" style="font-size: 16pt; margin: 0px; ">
				Titular {{ ($alumno->nacionalidad == 'V') ? 'de la Cédula de Identidad' : 'del Pasaporte'  }} Número {{ ($alumno->nacionalidad == 'V') ? $alumno->nacionalidad.'-'.$alumno->cedula : $alumno->cedula  }}, natural de <span style="text-transform: capitalize !important;">{{ mb_strtolower( $alumno->lugarn,"UTF-8" ) }}</span>, {{ $retVal = ($alumno->sexo == 'M') ? 'Nacido' : 'Nacida'  }} el {{ Carbon::parse($alumno->fechan)->format('d/m/Y')}}, aspirante al Título de
			...
			@else
			<div style="font-size: 30pt; /*padding-top: 30px; padding-bottom: 30px;*/ text-align: center; margin-top: 0px;">
			@endif
				<center>
					@if ($graduando->titulo == 1)
						@php
							echo $retVal = ($alumno->sexo == 'M') ? '<span class="inicial">T</span>écnico <span class="inicial">S</span>uperior <span class="inicial">U</span>niversitario en:' : '<span class="inicial">T</span>écnica <span class="inicial">S</span>uperior <span class="inicial">U</span>niversitaria en:';
						@endphp
			...
			<p align="center" style="font-size: 16pt; margin: 0px; margin-top:5px !important">
				Tómese razón de este título en la Institución y reconózcase y téngase en toda la República {{ $retVal = ($alumno->sexo == 'M') ? 'al' : 'a la'  }} Ciudadan{{ $retVal = ($alumno->sexo == 'M') ? 'o' : 'a'  }} {{ ucwords( mb_strtolower(' '.$alumno->p_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->s_nombre,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->p_apellido,"UTF-8" )) }} {{ ucwords(mb_strtolower(' '.$alumno->s_apellido,"UTF-8" )) }}, como tal
				@if ($graduando->titulo == 1)
					{{ $retVal = ($alumno->sexo == 'M') ? 'Técnico Superior  Universitario' : 'Técnica Superior  Universitaria'  }}
				@elseif ($graduando->titulo == 3)
					Técnico Superior  Universitario
				...
				@else
					en
				@endif
				{{$pnf}} con todas las facultades y derechos que le otorgan las Leyes y Reglamentos.
				<br>
				En fé de lo cual Firmo el presente título en unión de los Responsables: Director, Subdirectora y un Profesor. En Ciudad Bolívar, a los {{ strtolower($dia) }} días del mes de {{ strtolower($mes) }} del año dos mil {{ strtolower($graduando->numero_letras(Carbon::parse($graduando->egreso)->format('y'))) }}. Año {{Carbon::parse($graduando->egreso)->format('Y') - 1810 }} de la Independencia y {{Carbon::parse($graduando->egreso)->format('Y') - 1859 }} de la Federación.
			</p>
			<br>
			<br>
					...
					<tr>
						<td width="50%">
							<p style="font-size: 10pt !important">
								...
								<br>
								<span style="margin-left: 70px">de</span> <span style="margin-left: 70px">de 20</span>  <span style="margin-left: 40px"> Años 20 </span> <span style="margin-left: 40px"> y 15 </span>
								<br>
