   ...
   <div class="mb-2 row">
      ...
      <div class="col-sm-6">
         <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item">
               <a href="{{ route('panel.inscripciones.ciu.index') }}">
...
{{--
										...
										{{-- @endif --}}
										@if($a_cursar[@$asignatura->codigo] != 'ADELANTAR')
										<tr  class="bg-{{ (count($asignatura->RelacionSeccionDocente()) <= 0) ? 'gray' : 'primary' }}">
											<td scope="row">{{ $n++ }}</td>
											<td>{{ @$asignatura->Trayecto->nombre }}</td>
											<td>{{ @$asignatura->nombre }}  - <span class="badge badge-{{ ($a_cursar[@$asignatura->codigo] == 'ADELANTAR') ? 'danger' : 'success' }}"> {{ $a_cursar[@$asignatura->codigo] }}</span></td>
											<td>
												...
												<label class="form-check-label" for="{{ $asignatura->codigo }}" style="cursor:pointer">
													<input class="form-check-input check-asignatura" type="checkbox" name="uc_a_inscribir[{{$asignatura->id}}][]" id="{{ $asignatura->codigo }}" value="{{ $asignatura->id }}" {{ (count($asignatura->RelacionSeccionDocente()) <= 0) ? 'disabled' : 'checked' }}> INSCRIBIR
												</label>

											</td>
											<td>
												{{-- <div class="form-group"> --}}
													<select class="form-control form-control-sm" name="seccion[{{$asignatura->id}}]" id="">
														@forelse ($asignatura->RelacionSeccionDocente() as $relacion)
														@if ( @$relacion->Seccion->cupos > 0)
						...
						<div class="row">
							...
							<div class="text-right col-md-6">
								@if(!isset($ocultar))
									<input type="hidden" name="ciu" value="1">
