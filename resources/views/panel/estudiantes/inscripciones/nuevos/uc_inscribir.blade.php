        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                ...
                <div class="grid grid-cols-1 bg-gray-200 bg-opacity-25 md:grid-cols-1">
					...
					<div class="">
						<form action="{{ route('panel.estudiante.inscripciones.nuevo-ingreso.guardar') }}" method="post">
							@csrf
							@method('post')
							<input type="hidden" name="alumno_id" value="{{ $alumno->id }}" readonly>
							<input type="hidden" name="seccion_id" value="{{ $seccion->id }}" readonly>
							<table class="w-full table-auto min-w-max">
								...
								<tbody class="text-sm font-light text-gray-600">
									@php
										...
										$trayecto = 0;
									@endphp
									@forelse ($seccion->Plan->Asignaturas as $key => $asignatura)
										...
										@endif
											<tr  class="border-b border-gray-600 text-white bg-{{ $color=(1 <= 0) ? 'gray-600' : 'blue-900' }}">
												...
												<td class="px-6 py-3 text-left whitespace-nowrap">
													<span class=" hidden px-3 py-1 text-xs text-{{ $color=(1 <= 0) ? 'red-200' : 'green-200'}} bg-{{ $color=(1 <= 0) ? 'red-600' : 'green-600'}} rounded-full">
														<label class="tracking-wider form-check-label" for="{{ $asignatura->codigo }}" style="cursor:pointer">
															<input class="form-check-input check-asignatura" type="checkbox" name="uc_a_inscribir[{{$asignatura->trayecto_id}}][]" id="{{ $asignatura->codigo }}" value="{{ $asignatura->id }}" {{ (1 <= 0) ? 'disabled' : 'checked' }}> INSCRIBIR
														</label>
													</span>
												</td>
											</tr>
														...
														<tbody>
															@foreach ($asignatura->DesAsignaturas as $key => $desasignatura)
																<tr class="text-white bg-indigo-400 border-b border-indigo-200 hover:text-gray-700 hover:bg-indigo-100">
																	<td class="w-1/4 px-6 py-3 font-extrabold tracking-wide text-left whitespace-nowrap"><b>{{ $asignatura->Plan->cohorte }}</b> {{ @$desasignatura->tri_semestre }}</td>
