            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                ...
                <div class="grid grid-cols-1 bg-gray-200 bg-opacity-25 md:grid-cols-1">
					...
					<div class="overflow-x-auto">
						...
						<hr>
						<form action="{{ route('panel.auxiliar.inscripciones.nuevo-ingreso.store') }}" method="post">
							@csrf
							@method('post')
							<input type="hidden" name="alumno_id" value="{{ $alumno->id }}" readonly>
							<table class="w-full table-auto min-w-max">
								<thead>
									<tr class="text-sm leading-normal text-blue-600 uppercase bg-blue-200">
								...
								<tbody class="text-sm font-light text-gray-600">
									...
									@forelse ($uc_acursar as $key => $asignatura)
										...
										@if ($nota_final <= $aprueba)
												...
												<td   class="px-6 py-3 text-center whitespace-nowrap">
													{{-- <div class="form-group"> --}}
														<x-select class="block w-full mt-1" name="seccion[{{$asignatura->trayecto_id}}]" id=""  >
															...
															@if (@$relacion->Seccion->cupos > 0 && @$relacion->Seccion->nucleo_id == $alumno->nucleo_id && @$relacion->Seccion->estatus == "ACTIVA")
																<option value="{{ @$relacion->Seccion->id }}">{{ @$relacion->Seccion->nombre }} Cupos: {{ $relacion->Seccion->cupos }}</option>
																@php
											...
											@endif
											<tr  class="border-b border-gray-600 text-white bg-{{ $color=(count($asignatura->RelacionSeccionDocente()) <= 0 || $seccion_disponible == false) ? 'gray-600' : 'blue-900' }}">
												...
												<td class="px-6 py-3 text-left whitespace-nowrap">
													<span class="px-3 py-1 text-xs text-{{ $color=(count($asignatura->RelacionSeccionDocente()) <= 0 || $seccion_disponible == false) ? 'red-200' : 'green-200'}} bg-{{ $color=(count($asignatura->RelacionSeccionDocente()) <= 0 || $seccion_disponible == false) ? 'red-600' : 'green-600'}} rounded-full">
														<label class="tracking-wider form-check-label" for="{{ $asignatura->codigo }}" style="cursor:pointer">
															<input class="form-check-input check-asignatura" type="checkbox" name="uc_a_inscribir[{{$asignatura->trayecto_id}}][]" id="{{ $asignatura->codigo }}" value="{{ $asignatura->id }}" {{ (count($asignatura->RelacionSeccionDocente()) <= 0 || $seccion_disponible == false) ? 'disabled' : 'checked' }} onclick="return false;"> INSCRIBIR
														</label>
													</span>
												</td>
											</tr>
