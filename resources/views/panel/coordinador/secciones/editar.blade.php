            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                ...
                <div class="grid grid-cols-1 bg-gray-200 bg-opacity-25 md:grid-cols-1">
                    <div class="p-6 overflow-x-auto">
						<form action="{{ route('panel.coordinador.secciones.actualizar_config') }}" method="post" autocomplete="off">
							@csrf
							@method('post')
							...
							<table class="w-full table-auto min-w-max">
								...
								<tbody>
									...
									<tr class="bg-{{ ($seccion->checkAsignatura($asignatura->id) == true) ? 'green-600' :'red-800' }}">
										<th class="py-3 text-white" colspan="4" style="text-align: center;"  for="{{  $asignatura->codigo }}">
										<label for="{{  $asignatura->codigo }}" style="width: 95%; cursor: pointer; height: 100%;;" @can('jefe-pnf.secciones.edit-asignaturas') class="check-asignatura" @endcan >
												<input type="checkbox" name="n[]" {{ ($seccion->checkAsignatura($asignatura->id) == true) ? 'checked="true"' :''}} hidden="" class="hidden">
											{{  $asignatura->nombre }}
										</label>
										<input type="checkbox" {{ ($seccion->checkAsignatura($asignatura->id) == true) ? 'checked="true"' :''}} id="{{  $asignatura->codigo }}" value="{{ $asignatura->id }}" name="asignaturas_ins[]" hidden="true" class="hidden">
										</th>
