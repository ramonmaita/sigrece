							...
							<select name="" id="" class="form-control" wire:model="periodo_id">
								...
								@if (!is_null($estudiante))
									@forelse ($periodos as $periodo)
										<option value="{{ $periodo->id }}">({{ $periodo->nro_periodo }}) {{ $periodo->periodo }}</option>
									@empty
						...
						<div class="col-md-4">
							...
							<select name="" id="" class="form-control" wire:model="cod_anual">
								...
								@if (!is_null($periodo_id))
									@forelse ($ucs as $uc)
										<option value="{{ $uc->Asignatura->codigo }}">({{ $uc->Asignatura->Trayecto->nombre }}) {{ $uc->Asignatura->nombre }}</option>
									@empty
									...
									<div class="input-group form-group">
										...
										<div class="input-group-prepend">
										  <span class="input-group-text">
											<input type="checkbox" wire:model="cohortes_actualizar" value="{{ $unidad->id }}">
										  </span>
										</div>
										<input type="number" wire:model.defer="nota_cohorte.{{$unidad->id}}"
										class="form-control" @if (array_search($unidad->id, $cohortes_actualizar) === false) disabled="disabled"  value="0" @endif min="1" max="20" placeholder="{{ $unidad->nota }}">
			...
			<div class="card-body">
						...
						<select name="" id="" class="form-control" wire:model="cod_anual">
							...
							@if (!is_null($periodo_id))
								@forelse ($alumno->Plan->Asignaturas as $uc)
									<option value="{{ $uc->id }}">({{ $uc->Trayecto->nombre }}) {{ $uc->nombre }}</option>
								@empty
				...
				@if (!is_null($cohortes))
					...
					@foreach ($unidades as $unidad)
						<div class="row">
							...
							<div class="col-md-3">
								...
								<input type="text" maxlength="20" class="form-control" wire:model.defer="seccion.{{$unidad->id}}">
								@error("seccion")
									<small class="text-danger">{{ $message }}</small>
								@enderror
							...
							<div class="col-md-3">
								...
								<select name="" id="" class="form-control"  wire:model.defer="docente_id.{{$unidad->id}}">
									<option value="">Seleccione</option>
					...
					<div class="row">
						...
						<div class="col-md-3">
							{{-- @if (count($cohortes_actualizar) > 0) --}}
								<button type="button" name="" id="" class="btn btn-primary btn-block" wire:click="store">
									Agregar
								</button>
							{{-- @endif --}}
						</div>
